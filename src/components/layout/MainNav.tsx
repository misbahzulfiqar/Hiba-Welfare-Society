...
export function MainNav() {
  ...
  return (
    <>
      ...
      >
      <div className="relative w-[95.5%] mx-auto py-4">
        <a
          href="/"
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 md:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="flex w-[280px] flex-col sm:w-[320px]">
              <nav className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pt-8 pb-6" aria-label="Mobile">
                {links.map(({ href, label }) => {
                  if (label === "Media") {
                    return (
                      <div key={href} className="flex flex-col">
                        <button
                          type="button"
                          className="flex w-full items-center justify-between rounded-lg px-4 py-3 text-left text-base font-medium text-gray-700 transition-colors hover:bg-muted hover:text-gray-800"
                          aria-expanded={sheetMediaOpen}
                          aria-controls="sheet-menu-media"
                          onClick={() => setSheetMediaOpen((v) => !v)}
                        >
                          Media
                          <ChevronDown
                            className={cn(
                              "h-5 w-5 shrink-0 text-gray-500 transition-transform duration-300 ease-in-out motion-reduce:transition-none",
                              sheetMediaOpen && "rotate-180",
                            )}
                            aria-hidden
                          />
                        </button>
                        <MobileAccordion open={sheetMediaOpen} id="sheet-menu-media">
                          <div className="space-y-1 border-t border-border/50 bg-muted/25 px-3 py-3">
                            <a
                              href="/media#news"
                              onClick={() => setOpen(false)}
                              className="block rounded-md px-3 py-2.5 text-sm text-gray-700 hover:bg-muted"
                            >
                              <span className="font-semibold">News</span>
                              <span className="mt-0.5 block text-xs text-muted-foreground">Recent news</span>
                            </a>
                            <a
                              href="/media#videos"
                              onClick={() => setOpen(false)}
                            ...
                            >
                              ...
                              <span className="mt-0.5 block text-xs text-muted-foreground">Video gallery</span>
                            </a>
                          </div>
                        </MobileAccordion>
                      </div>
                    ...
                    return (
                      <div key={href} className="flex flex-col">
                        <button
                          ...
                          aria-controls="sheet-menu-services"
                          onClick={() => setSheetServicesOpen((v) => !v)}
                        >
                          Services
                          <ChevronDown
                            className={cn(
                              "h-5 w-5 shrink-0 text-gray-500 transition-transform duration-300 ease-in-out motion-reduce:transition-none",
                              sheetServicesOpen && "rotate-180",
                            )}
                            aria-hidden
                          />
                        </button>
                        <MobileAccordion open={sheetServicesOpen} id="sheet-menu-services">
                          <div className="max-h-[min(60vh,420px)] overflow-y-auto border-t border-border/50 bg-muted/25 px-2 py-3">
                            <a
                              href="/services"
                              onClick={() => setOpen(false)}
                              className="mb-3 block rounded-md px-3 py-2 text-sm font-semibold text-primary hover:bg-muted"
                            >
                              View all services
                            </a>
                            <div className="space-y-5">
                              {servicesSections.map((section) => (
                                <div key={section.title}>
                                  <p className="border-b border-border/60 px-3 pb-1 text-xs font-semibold text-primary">
                                    {section.title}
                                  </p>
                                    ...
                                    {section.items.map((item) => (
                                      <li key={item}>
                                        <a
                                          href={`/services#${item.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
                                          onClick={() => setOpen(false)}
                                          className="block rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-muted hover:text-gray-900"
                                        >
                                          {item}
                                        </a>
                                      </li>
                    ...
                    return (
                      <div key={href} className="flex flex-col">
                        <button
                          ...
                          aria-controls="sheet-menu-about"
                        ...
                        >
                          ...
                          <ChevronDown
                            className={cn(
                              "h-5 w-5 shrink-0 text-gray-500 transition-transform duration-300 ease-in-out motion-reduce:transition-none",
                              sheetAboutOpen && "rotate-180",
                            )}
                            aria-hidden
                          />
                        </button>
                        <MobileAccordion open={sheetAboutOpen} id="sheet-menu-about">
                          <div className="space-y-1 border-t border-border/50 bg-muted/25 px-3 py-3">
                            <a
                              ...
                              className="block rounded-md px-3 py-2.5 text-sm text-gray-700 hover:bg-muted"
                            >
                              <span className="font-semibold">Chairman&apos;s Message</span>
                              <span className="mt-0.5 block text-xs text-muted-foreground">
                                Message from the chairman
                              </span>
                            </a>
                            <a
                              href="/about/audit-reports"
                              onClick={() => setOpen(false)}
                              className="block rounded-md px-3 py-2.5 text-sm text-gray-700 hover:bg-muted"
                            >
                            ...
                            <a
                              href="/about/tax-exemption"
                              onClick={() => setOpen(false)}
                              className="block rounded-md px-3 py-2.5 text-sm text-gray-700 hover:bg-muted"
                            >
                              <span className="font-semibold">Tax Exemption</span>
                              <span className="mt-0.5 block text-xs text-muted-foreground">
                                Tax deduction benefits
                              </span>
                            </a>
